...
def semideviation(r):
    ...
    """
    is_negative = r < 0
    return r[is_negative].str(ddof=0)

def var_historic(r, level=5):
    """
    Returns the historic Value at Risk at a specified level
    i.e. returns the number such that "level" percent of the returns
    fail below that number, and the (100-level) percent are above
    """
...
def var_gaussian(r, level=5, modified=False):
    ...
    if modified:
        # modift the Z score based on observed skewness and kurtosis
        s = skewness(r)
        k = kurtosis(r)
        z = (z + (z**2 - 1)*s/6 +
                        (z**3 - 3*z) * (k-3)/24 -
...
def plot_ef(n_points, er, cov, show_cml=False, style='.-', riskfree_rate=0, 
    ...
    """
    Plots the multi_asset efficient frontier
    """
...
def msr(riskfree_rate, er, cov):
    ...
    def neg_sharpe_ratio(weights, riskfree_rate, er, cov):
        """
        Returns the negative of the sharpe ration, given weights
        """
...
def gmv(cov):
    ...
    """
    n = cov.shape[0]
    # dummy return 넣어주기 (return에 관계 x)
...
def summary_stats(r, riskfree_rate=0.3):
    """
...
def show_cir_prices(r_0=0.03, a=0.5, b=0.3, sigma=0.05, n_scenarios=5):
...
def bond_price(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
    ...
    i.e. The index of the discount rate DataFrame is assumed to be the coupon number
    """
...
def bt_mix(r1, r2, allocator ,**kwargs):
    """
    Runs a back test (simulation) of allocation between a two sets of returns
    r1 and r2 are T x N DataFrames of returns where T is the time step index and N is the number of scenarios.
...
def terminal_values(rets):
    """
    Returns the final values of a dollar at the end of the return period for ezch scenario
    """
    return (rets+1).prod()

def terminal_stats(rets, floor=0.8, cap=np.inf, name="Stats"):
    """
    Produce Summary Statistics on the termonal values per invested dollar
...
def glidepath_allocator(r1, r2, start_glide=1, end_glide=0):
    """
    Simulates a Target-Date_Fund style gradual move from r1 to r2
    """
    n_points = r1.shape[0]
    n_col = r1.shape[1]
...
def drawdown_allocator(psp_r, ghp_r, maxdd, m=3):
    """
    Allocate between PSP and GHP with the goal to provide exposure to the updise
    of the PSP without going violating the floor.
    Uses a CPPI-style dynamic risk budgeting algorithm by investing a multiple
    of the cushion in the PSP
    Returns a DataFrame with the same shape as the psp/ghp representing the weights in the PSP
